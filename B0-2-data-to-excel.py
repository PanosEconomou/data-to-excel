...
def pick_time_save():
    questions = [
        inquirer.List('time_save',
                      message="Κάθε πόσα δευτερόλεπτα να αποθηκεύεται το αρχείο;",
...
def record_data(ser, sheet, wb, times, values, saving_time):
    ...
    try:
        while True:
            ...
            if line:  # Αν δεν είναι κενό το data
                ...
                if time.time() - last_save_time >= saving_time:
            ...
            # Χρόνος αναμονής για να αποφύγουμε τη συγχώνευση καταγραφών (μειωμένο)
            #time.sleep(0.05)
...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    ani = FuncAnimation(fig, animate, fargs=(times, values, ax), interval=2000)  # Αυξήσαμε το διάστημα ανανέωσης

    # Ξεκινάμε τη διαδικασία καταγραφής δεδομένων σε νέο νήμα
    record_thread = threading.Thread(target=record_data, args=(ser, sheet, wb, times, values, saving_time))
